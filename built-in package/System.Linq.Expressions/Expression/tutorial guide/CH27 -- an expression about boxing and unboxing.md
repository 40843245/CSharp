...
## CH27.2 -- an expression about boxing
There are NO one static method in `System.Linq.Expression.Expression` to create an expression that boxes value.

But, we can made it by myself.

Step 1:

define an anonymyous instance (i.e. `AnonymyousType` instance)

```
var anonymousInstance = new 
{ 
    Value1 = default(int) , 
    Value2 = default(string) 
}
;
```
...
Get type of the anonymyous instance.
...
create the corresponding anonymous type.
...
create an expression about `new` expression.
...
convert the expreesion to type `Object`.

To do that, we can invoke `Expression.Convert` static method.
...
compile the lamda expression and execute it.

```
directBoxingLambda.Compile().Invoke();
```
...
```
In TestMethod48 method call,
{ Value1 = 20, Value2 = World }
```
...
+ The code in example 2 are generated by [Google Gemini Flash 2.5](https://g.co/gemini/share/3d201e10b65f) and modified by the author.

+ The code in example 3 are generated by [Google Gemini Flash 2.5](https://g.co/gemini/share/84e5aea484c5) and modified by the author.